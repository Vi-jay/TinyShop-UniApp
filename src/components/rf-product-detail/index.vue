	...
	<view class='rf-product-detail'>
		...
		<view class='detail' v-if='product.name'>
				...
				<view class='introduce-first-line'>
					...
					<view class='price-box' v-else>
						...
						<view class='m-price-wrapper' v-if='product.market_price > product.price'>
							价格
							<text class='m-price'>{{ moneySymbol }}{{ product.market_price }}</text>
						</view>
					</view>
			...
			<view class='c-list'>
				...
				>
					<view slot='content'>
						<text class='selected-text' v-if='currentSkuName === singleSkuText'>{{ currentCartCount }}
							{{ product.unit || '件' }}
						</text>
				...
				>
					...
					<view slot='right' v-if='product.canReceiveCoupon.length > 0'>
						<text class='iconfont iconyou'></text>
					</view>
					<view slot='popup' class='service'>
						<!-- 优惠券列表 -->
				...
				<rf-item-popup title='积分活动' v-if="product.point_exchange_type !== '1'">
					<view slot='content' class='con-list'>
						<text v-if='product.point_exchange_type'>兑换类型: {{ product.point_exchange_type | pointExchangeTypeFilter }}
						</text>
						<text v-if='parseInt(product.give_point, 10) > 0'>赠送类型:
							{{ product.integral_give_type | integralGiveTypeFilter }}
						</text>
...
.rf-product-detail {
	...
	.carousel {
		...
		.swiper-item {
			...
			height: 750 upx;
			overflow: hidden;
			border-bottom: 1 upx solid rgba(0, 0, 0, 0.01);
			image {
				width: 100%;
				height: 100%;
			}
	...
	.layer {
		...
		bottom: 0;
		width: 100%;
		border-radius: 10 upx 10 upx 0 0;
